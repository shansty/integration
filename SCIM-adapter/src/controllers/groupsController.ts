...
import { extractUserIdsFromMembersFilterPath, extractUserIdsFromPatchValue, findGroupByAnyId, scimError } from './helpers/groupHelpers';
...
export async function getGroupById(req: Request, res: Response) {
  ...
  try {
    ...
    console.dir({ groupRecord }, { depth: null });
...
export async function createGroup(req: Request, res: Response) {
  try {
    ...
    const brivoId: string = String(brivoGroup.id ?? brivoGroup.groupId ?? '');
...
export async function patchGroup(req: Request, res: Response) {
  try {
    ...
    for (const operation of operations) {
      ...
      if (operationType === 'remove') {
        if (normalizedPath.startsWith('members[')) {
          // e.g. members[value eq "userId"] (most common from Okta UI)
          ...
          if (explicitIds.length > 0) {
            userIdsToRemove.push(...explicitIds); // remove listed ids
          } else {
            // SCIM-compliant: path=members with no value => clear ALL members
            userIdsToRemove.push(...existingGroup.members.map(memberRecord => memberRecord.userId));
          }
        }
        continue;
      }
      ...
      if (operationType === 'replace' && normalizedPath === 'members') {
        ...
        const existingMemberIds = new Set(existingGroup.members.map(memberRecord => memberRecord.userId));
        for (const userId of newMemberIds) if (!existingMemberIds.has(userId)) userIdsToAdd.push(userId);
        for (const userId of existingMemberIds) if (!newMemberIds.has(userId)) userIdsToRemove.push(userId);
        continue;
      }
    ...
    if (groupFieldUpdates.displayName !== undefined) {
      await prisma.group.update({ where: { id: groupId }, data: { displayName: groupFieldUpdates.displayName } });
      if (existingGroup.brivoId) {
        await brivo.updateGroup(req, existingGroup.brivoId, { name: groupFieldUpdates.displayName });
      }
    }
  ...
  try {
    ...
    if (existingGroup.brivoId) {
      try {
        ...
        console.warn('deleteGroup: brivo.deleteGroup failed (continuing):', existingGroup.brivoId, externalError);
      }
    }
